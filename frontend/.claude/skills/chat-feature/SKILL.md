...
- `2026-06-16 · Phase 1: Wire data thật cho SelectFriendBottomSheet (FAB "+" → friend list → mở chat)` · files: `app/src/main/java/com/example/frontend/ui/chat/FriendPickAdapter.java`, `ui/chat/SelectFriendBottomSheet.java`, `ui/chat/ChatFragment.java`, `app/src/main/res/layout/sheet_select_friend.xml`
  - Note: Tạo `FriendPickAdapter` (RecyclerView) bind `item_friend_pick.xml` — avatar Glide circleCrop + placeholder `ic_user` + username. Interface `OnFriendClickListener.onFriendClick(Friend)`. `SelectFriendBottomSheet` giờ gọi `ApiService.getFriends()` qua `ApiClient.getApiService(appContext)` (enqueue, cancel ở `onDestroyView`), state machine: loading (ProgressBar) → list / empty ("Chưa có bạn bè" hoặc "Không tìm thấy bạn bè phù hợp"). Search: `TextWatcher` lọc `allFriends` theo `username.toLowerCase().contains(query)`. Click row → `dismiss()` rồi cast `getParentFragment()` thành `ChatFragment` và gọi public method mới `openChatWith(String userId)` (reuse `viewModel.openConversation()` + observer `getOpenConversationResult()` đã navigate sang `ChatDetailFragment`). Layout: thêm `progressFriendPick` + id `tvEmptyFriendPick`, empty mặc định `visibility="gone"` (state quyết định lúc runtime). Build: `./gradlew :app:compileDebugJavaWithJavac` PASS (JAVA_HOME=D:/Android/jbr, JDK 21).
